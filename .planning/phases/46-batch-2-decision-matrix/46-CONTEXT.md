...
- Exact wording of `phase47_instruction` enum strings for mechanical promote/alias (planner may align with Phase 42 parser conventions).
- Whether to add optional `expected_effect` and `notes` columns beyond the required set (recommended for parity with `41-DECISION-MATRIX.md`).
