...
- `src/cli/compile.ts` exposes the official compile CLI; it supports `--version`, `--out`, `--generated-at`, and `--quality-report`. The `parseCompileArgs` parser (`src/cli/parse_args.ts:43`) maps `--out` to `outputDir` and accepts **no alias** — `--output` is not a valid flag (Phase 47 docs used it in error). The `resolveOutputDir` helper resolves the default `data/compiled/v2` to `../data/compiled/v2` when invoked from the `src/` package.
